#include <stdio.h>
#include <stdint.h>
#include <stdlib.h>
#include <string.h>
#include <errno.h>
#include <unistd.h>
#include <arpa/inet.h>
#include <sys/socket.h>
#include <netinet/in.h>
#include <event2/event.h>

#include <velib/platform/plt.h>
#include <velib/types/ve_item.h>
#include <velib/utils/ve_item_utils.h>

#include "ble-dbus.h"
#include "ble-scan.h"
#include "ble-socket.h"
#include "task.h"

#define BLE_SOCKET_MAX_PACKET	128
#define BLE_SOCKET_MIN_SIZE	11

struct ble_socket {
	int			sock;
	struct event		*ev;
	struct sockaddr_in	bind_addr;
};

static struct ble_socket ble_sock = {
	.sock = -1,
};

static struct VeSettingProperties enabled_props = {
	.type		= VE_SN32,
	.def.value.SN32	= 0,
	.min.value.SN32	= 0,
	.max.value.SN32	= 1,
};

static struct VeSettingProperties port_props = {
	.type		= VE_SN32,
	.def.value.SN32	= BLE_SOCKET_DEFAULT_PORT,
	.min.value.SN32	= 1,
	.max.value.SN32	= 65535,
};

static struct VeSettingProperties bind_props = {
	.type		= VE_HEAP_STR,
	.def.value.Ptr	= BLE_SOCKET_DEFAULT_BIND,
};

static void ble_socket_stop(void)
{
	if (ble_sock.ev) {
		event_free(ble_sock.ev);
		ble_sock.ev = NULL;
	}

	if (ble_sock.sock >= 0) {
		close(ble_sock.sock);
		ble_sock.sock = -1;
		fprintf(stderr, "ble-socket: stopped\n");
	}
}

static int ble_socket_parse(const uint8_t *buf, int len)
{
	uint8_t addr[6];
	uint8_t version;
	uint8_t flags;
	uint16_t mfg_id;
	uint8_t payload_len;
	const uint8_t *payload;
	int offset;

	if (len < BLE_SOCKET_MIN_SIZE) {
		fprintf(stderr, "ble-socket: packet too short (%d < %d)\n",
			len, BLE_SOCKET_MIN_SIZE);
		return -1;
	}

	version = buf[0];
	if (version != BLE_SOCKET_VERSION) {
		fprintf(stderr, "ble-socket: unknown version %d\n", version);
		return -1;
	}

	flags = buf[1];

	memcpy(addr, buf + 2, 6);

	mfg_id = buf[8] | (buf[9] << 8);
	payload_len = buf[10];
	payload = buf + 11;
	offset = 11 + payload_len;

	if (len < offset) {
		fprintf(stderr, "ble-socket: payload truncated (%d < %d)\n",
			len, offset);
		return -1;
	}

	if (flags & BLE_SOCKET_FLAG_RSSI) {
		if (len < offset + 1)
			return -1;
		offset += 1;
	}

	if (flags & BLE_SOCKET_FLAG_REPEATER) {
		if (len < offset + 6)
			return -1;
		offset += 6;
	}

	return ble_handle_mfg(addr, mfg_id, payload, payload_len, BLE_SOURCE_SOCKET);
}

static void ble_socket_read(evutil_socket_t fd, short events, void *arg)
{
	uint8_t buf[BLE_SOCKET_MAX_PACKET];
	struct sockaddr_in src;
	socklen_t srclen = sizeof(src);
	int len;

	len = recvfrom(fd, buf, sizeof(buf), 0,
		       (struct sockaddr *)&src, &srclen);
	if (len < 0) {
		if (errno != EAGAIN)
			fprintf(stderr, "ble-socket: recvfrom: %s\n",
				strerror(errno));
		return;
	}

	ble_socket_parse(buf, len);
}

static int ble_socket_start(const char *bind_addr, int port)
{
	struct event_base *base;
	int sock;
	int err;

	if (ble_sock.sock >= 0)
		return 0;

	sock = socket(AF_INET, SOCK_DGRAM, 0);
	if (sock < 0) {
		fprintf(stderr, "ble-socket: socket: %s\n", strerror(errno));
		return -1;
	}

	memset(&ble_sock.bind_addr, 0, sizeof(ble_sock.bind_addr));
	ble_sock.bind_addr.sin_family = AF_INET;
	ble_sock.bind_addr.sin_port = htons(port);

	err = inet_pton(AF_INET, bind_addr, &ble_sock.bind_addr.sin_addr);
	if (err != 1) {
		fprintf(stderr, "ble-socket: invalid bind address: %s\n",
			bind_addr);
		close(sock);
		return -1;
	}

	err = bind(sock, (struct sockaddr *)&ble_sock.bind_addr,
		   sizeof(ble_sock.bind_addr));
	if (err < 0) {
		fprintf(stderr, "ble-socket: bind: %s\n", strerror(errno));
		close(sock);
		return -1;
	}

	base = pltGetLibEventBase();
	ble_sock.ev = event_new(base, sock, EV_READ | EV_PERSIST,
				ble_socket_read, NULL);
	if (!ble_sock.ev) {
		fprintf(stderr, "ble-socket: event_new failed\n");
		close(sock);
		return -1;
	}

	event_add(ble_sock.ev, NULL);
	ble_sock.sock = sock;

	fprintf(stderr, "ble-socket: listening on %s:%d\n", bind_addr, port);

	return 0;
}

static void ble_socket_update(void)
{
	struct VeItem *ctl = get_control();
	struct VeItem *item;
	VeVariant val;
	const char *bind_addr;
	int enabled;
	int port;

	enabled = veItemValueInt(ctl, "Socket/Enabled");
	port = veItemValueInt(ctl, "Socket/Port");

	item = veItemByUid(ctl, "Socket/BindAddress");
	if (item && veItemIsValid(item)) {
		veItemLocalValue(item, &val);
		bind_addr = val.value.Ptr;
	} else {
		bind_addr = BLE_SOCKET_DEFAULT_BIND;
	}

	if (!enabled) {
		ble_socket_stop();
		return;
	}

	if (ble_sock.sock >= 0)
		ble_socket_stop();

	ble_socket_start(bind_addr, port);
}

static void on_socket_setting_changed(struct VeItem *item)
{
	ble_socket_update();
}

int ble_socket_init(void)
{
	struct VeItem *settings = get_settings();
	struct VeItem *ctl = get_control();
	struct VeItem *item;

	item = veItemCreateSettingsProxy(settings, "Settings/BleSensors",
		ctl, "Socket/Enabled", veVariantFmt, &veUnitNone,
		&enabled_props);
	veItemSetChanged(item, on_socket_setting_changed);

	item = veItemCreateSettingsProxy(settings, "Settings/BleSensors",
		ctl, "Socket/Port", veVariantFmt, &veUnitNone,
		&port_props);
	veItemSetChanged(item, on_socket_setting_changed);

	item = veItemCreateSettingsProxy(settings, "Settings/BleSensors",
		ctl, "Socket/BindAddress", veVariantFmt, &veUnitNone,
		&bind_props);
	veItemSetChanged(item, on_socket_setting_changed);

	return 0;
}

void ble_socket_close(void)
{
	ble_socket_stop();
}
